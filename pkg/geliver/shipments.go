...
func (c *Client) GetShipment(ctx context.Context, shipmentID string) (*Shipment, error) {
    var out Shipment
    if err := c.do(ctx, "GET", "/shipments/"+url.PathEscape(shipmentID), nil, nil, &out); err != nil { return nil, err }
    return &out, nil
}
...
func (c *Client) ListShipments(ctx context.Context, p *ListParams) (*ListShipmentsResponse, error) {
    ...
    if err := c.do(ctx, "GET", "/shipments", q, nil, &out); err != nil { return nil, err }
    return &out, nil
}

func (c *Client) UpdatePackage(ctx context.Context, shipmentID string, body map[string]any) (*Shipment, error) {
    var out Shipment
    if err := c.do(ctx, "PATCH", "/shipments/"+url.PathEscape(shipmentID), nil, body, &out); err != nil { return nil, err }
    return &out, nil
}
...
func (c *Client) CancelShipment(ctx context.Context, shipmentID string) (*Shipment, error) {
    var out Shipment
    if err := c.do(ctx, "DELETE", "/shipments/"+url.PathEscape(shipmentID), nil, nil, &out); err != nil { return nil, err }
    return &out, nil
}

func (c *Client) CloneShipment(ctx context.Context, shipmentID string) (*Shipment, error) {
    var out Shipment
    if err := c.do(ctx, "POST", "/shipments/"+url.PathEscape(shipmentID), nil, nil, &out); err != nil { return nil, err }
    return &out, nil
}
...
func (c *Client) WaitForOffers(ctx context.Context, shipmentID string, interval time.Duration, timeout time.Duration) (map[string]any, error) {
    ...
    for {
        ...
        if s.Offers.PercentageCompleted >= 99 {
        ...
        if time.Since(start) > timeout { return nil, errors.New("timeout waiting for offers") }
        time.Sleep(interval)
    }
}
...
func (c *Client) WaitForTrackingNumber(ctx context.Context, shipmentID string, interval time.Duration, timeout time.Duration) (*Shipment, error) {
    ...
    for {
        s, err := c.GetShipment(ctx, shipmentID)
        if err != nil { return nil, err }
        if s.TrackingNumber != "" { return s, nil }
        if time.Since(start) > timeout { return nil, errors.New("timeout waiting for tracking number") }
        time.Sleep(interval)
    }
}
...
func (c *Client) DownloadURL(ctx context.Context, url string) ([]byte, error) {
    ...
    if res.StatusCode >= 400 { return nil, fmt.Errorf("download error: %d", res.StatusCode) }
...
func (c *Client) DownloadShipmentLabel(ctx context.Context, shipmentID string) ([]byte, error) {
    s, err := c.GetShipment(ctx, shipmentID)
    if err != nil { return nil, err }
    if s.LabelURL == "" { return nil, errors.New("shipment has no labelURL") }
