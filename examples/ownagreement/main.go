...
func main() {
    ...
    if token == "" { fmt.Println("GELIVER_TOKEN is required"); return }
    c := g.NewClient(token)
    ...
    if err != nil || sender == nil { fmt.Println("sender error:", err); return }
    ...
    req := g.CreateShipmentWithRecipientAddress{
        ...
        RecipientAddress: g.Address{
            Name: "OwnAg Recipient", Address1: "Dest 2", CountryCode: "TR",
            CityName: "Istanbul", CityCode: "34", DistrictName: "Esenyurt",
        },
    }
    tx, err := c.CreateTransactionWithRecipientAddress(ctx, req)
    if err != nil { fmt.Println("create transaction (own agreement) error:", err); return }
